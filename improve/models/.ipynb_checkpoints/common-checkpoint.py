...
class SPPSCEP(nn.Module):
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            b = self.m[1](x)
            c = self.m[2](x)
            tpt = self.cv2(torch.cat([x] + [a] + [b] + [c], 1))
            tpt = self.CF(tpt)
            return tpt


class SPPAG(nn.Module):
    def __init__(self, c1, c2, k=(5, 9, 13)):  # 这里5，9，13，就是初始化的kernel size
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)  # 这里对应第一个CBL
        self.cv2 = Conv(c_ * (len(k) + 1), c2, 1, 1)  # 这里对应SPP操作里的最后一个CBL
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])
        self.c1 = Gnblock(640,640)
        self.c2 = Gnblock(640,640)
        self.c3 = Gnblock(640,640)
        self.CF = SCEP(1280,1,False)
...
class SCEP(nn.Module):
    def __init__(self, n_feat, kernel_size, bias):
        super(SCEP, self).__init__()
        self.conv1 = conv(n_feat, n_feat, kernel_size, bias=bias)
        self.conv2 = conv(n_feat, 1280, kernel_size, bias=bias)
        self.conv3 = conv(1280, n_feat, kernel_size, bias=bias)
